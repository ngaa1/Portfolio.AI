...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-600 font-sans selection:bg-brand-200 selection:text-brand-900">
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-40 transition-all duration-300 ${scrolled ? 'bg-white/95 backdrop-blur-md border-b border-slate-200 py-3 shadow-sm' : 'bg-transparent py-6'}`}>
        <div className="max-w-6xl mx-auto px-6 flex justify-between items-center">
          <div className="font-bold text-xl tracking-tight text-slate-900 flex items-center gap-2">
            <div className="bg-brand-800 text-white p-1.5 rounded-md shadow-sm">
              <DraftingCompass className="w-5 h-5" />
            </div>
            <span className="font-mono tracking-tighter">LI CHULONG<span className="text-brand-600">.STRUCT</span></span>
          </div>
          ...
          <a href="#contact" className="bg-brand-800 hover:bg-brand-900 text-white px-5 py-2 rounded text-sm font-medium transition-colors shadow-lg shadow-brand-900/20">
            联系我
          </a>
        </div>
      </nav>
        ...
        <div className="max-w-6xl mx-auto px-6 relative z-10 grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8 animate-in slide-in-from-left-5 fade-in duration-700">
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#projects" className="bg-brand-800 hover:bg-brand-900 text-white px-8 py-3.5 rounded font-semibold transition-all shadow-xl shadow-brand-900/10 flex items-center gap-2">
          ...
          <div className="relative hidden lg:block animate-in zoom-in-95 fade-in duration-1000 delay-200">
             ...
             <div className="bg-white border border-slate-200 rounded-xl p-2 shadow-2xl rotate-2 hover:rotate-0 transition-all duration-500 relative group">
                <div className="bg-slate-50 rounded-lg p-6 border border-slate-200 h-full relative overflow-hidden">
                ...
                <div className="absolute -right-4 -bottom-4 w-24 h-24 bg-brand-50 rounded-full border border-brand-200 flex items-center justify-center opacity-90 shadow-lg">
              ...
              <div key={job.id} className="relative pl-8 md:pl-0 group">
                ...
                <div className="grid md:grid-cols-[120px_1fr] gap-8 md:gap-12">
                  ...
                  <div className="relative pt-1 text-right hidden md:block">
                     ...
                     <div className="text-xs text-slate-400 mt-1">{job.period.split(' - ')[1] || '至今'}</div>
                  </div>
                            ...
                            <ul className="space-y-3">
                              {job.coreResponsibilities.map((resp, idx) => (
                                <li key={idx} className="text-sm text-slate-700 flex items-start gap-2.5">
                                  <div className="mt-1.5 w-1 h-1 rounded-full bg-brand-300 shrink-0" />
                                  {resp}
                                </li>
                              ))}
                            </ul>
                            ...
                            <div>
                               ...
                               <div className="space-y-3">
                                 {job.projects.map((proj, idx) => (
                                   <div key={idx} className="bg-slate-50 p-3 rounded border border-slate-100 hover:border-brand-200 transition-colors">
                  ...
                  <div key={project.id} className="group bg-white rounded-xl overflow-hidden border border-slate-200 hover:border-brand-300 transition-all hover:shadow-xl hover:shadow-slate-200/50 flex flex-col h-full">
                    <div className="h-48 overflow-hidden relative bg-slate-100">
                       <img 
                        ...
                        />
                       <div className="absolute bottom-0 left-0 right-0 h-1/2 bg-gradient-to-t from-slate-900/80 to-transparent" />
              ...
              <div>
                ...
                <div className="space-y-6">
                  {PROFILE_DATA.education.map((edu, idx) => (
                    <div key={idx} className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm relative overflow-hidden group hover:border-brand-200 transition-colors">
                      <div className="absolute top-0 right-0 w-16 h-16 bg-slate-50 rounded-bl-full -mr-8 -mt-8 transition-colors group-hover:bg-brand-50" />
      ...
      {/* Footer / Contact */}
      <footer id="contact" className="py-20 bg-slate-850 text-slate-300 border-t border-slate-800">
